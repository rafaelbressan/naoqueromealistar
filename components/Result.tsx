...
import { DURATION, EASE, EASE_KEYWORD, DISTANCE, SCALE, BLUR } from '@/lib/motion';

/**
 * Blocks rise in sequence rather than landing at once.
 *
 * Beyond the polish, the stagger buys a reading order. The legal basis is the
 * thing that makes the whole result mean anything, and today it competes for
 * attention with five sibling boxes of the same weight.
 */
const listVariants = {
  hidden: {},
  shown: {
    transition: { staggerChildren: DURATION.stagger, delayChildren: DURATION.quick },
  },
};
...
export function Result({ result, onRestart }: ResultProps) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0, scale: SCALE.large }}
    ...
    >
      ...
      >
        {/* Result header */}
        <motion.div variants={blockVariants} className="text-center space-y-4">
        ...
        {/* Reason */}
        {result.razao && (
          <motion.div variants={blockVariants} className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        {/* Legal basis */}
        {result.base_legal && (
          <motion.div variants={blockVariants} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
        ...
        {result.tipo_dispensa && (
          <motion.div variants={blockVariants} className="bg-green-50 border border-green-200 rounded-lg p-4">
        ...
        {/* Additional note */}
        {result.nota && (
          <motion.div variants={blockVariants} className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        ...
        {/* Tip */}
        {result.dica && (
          <motion.div variants={blockVariants} className="bg-purple-50 border border-purple-200 rounded-lg p-4">
        ...
        {/* Instructions */}
        {result.instrucao && (
          <motion.div variants={blockVariants} className="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
        ...
        {/* Alert */}
        {result.alerta && (
          <motion.div variants={blockVariants} className="bg-red-50 border border-red-200 rounded-lg p-4">
        ...
        {/* Disclaimer */}
        <motion.div variants={blockVariants} className="border-t pt-6 space-y-4">
